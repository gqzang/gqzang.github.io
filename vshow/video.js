...
const setStat = x => { loading = lBtn.disabled = x; de("cnt").innerText = 0; setLS(LV, getSec())
...
const loadList = () => vCat.forEach( x => de(x).innerText = '' ) || Object.keys(vdInfo).forEach(
...
vCat.forEach( x => de(x).onclick = 
    e => loading || setStat(false) || ( lBtn.innerText = LV + ( id = e.target.innerText ) ) )
setInterval( () => loading && ( de("cnt").innerText = ( getSec() - getLS(LV) ).toFixed(1) ), 100 )