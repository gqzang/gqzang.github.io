...
const loadV = async () => { setLoadStat(true); let ref = vdBkt[id.charAt(0)] + id.substring(0, 3) + '.xef'
    try { de('vid').src = vObjs[id] = URL.createObjectURL( await getBlob(loadPswd(), ref) )
        const nOpt = dc('option'); delete vdInfo[nOpt.value = nOpt.text = id]
...
const loadList = () => vCat.forEach( x => de(x).innerHTML = '' ) || Object.keys(vdInfo).forEach(
    k => {let x = dc('li'); x.textContent = k; x.setAttribute('tabindex', 1)
        de(k.charAt(0)).appendChild(x); loading = false; lBtn.innerText = LV });     loadList() // 1st time

vCat.forEach( x => de(x).onclick = e => { if(loading) return
    setLoadStat(false); id = e.target.innerHTML; lBtn.innerText = LV + id } )
